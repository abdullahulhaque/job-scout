...
def _build_html(jobs: pd.DataFrame, profile: dict) -> str:
    rows = ""
    for _, job in jobs.iterrows():
        ...
        posted   = str(job.get("date_posted", "")).split(" ")[0]
        url      = job.get("job_url", "#")
    ...
    <body style="font-family:Arial,sans-serif;max-width:960px;margin:0 auto;padding:20px;color:#333;">
      <h2 style="margin-bottom:4px;">🧭 Job Scout — {name}</h2>
      ...
      <p style="color:#bbb;font-size:11px;margin-top:24px;">Job Scout runs hourly via GitHub Actions.</p>
    </body>
    </html>
    """


def send_email(jobs: pd.DataFrame, profile: dict):
